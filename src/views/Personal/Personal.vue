<template>
  <div>
    <section class="profile border-1px">
      <Header title="我的"/>
    ...
    <section class="profile_my_order border-1px" v-if="user._id">
      <mt-button type="danger" style="width: 100%;margin-top: 20px" @click="logout">退出登录</mt-button>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    logout () {
      MessageBox.confirm('确定要退出么?')
        .then(action => {
          this.$store.dispatch('logout')
          Toast({
            message: '退出成功',
            iconClass: 'iconfont icon-secede'
          })
        })
    }
  }
